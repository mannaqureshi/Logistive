...
const SectionList = ({ place, sections }) => {
  ...
  const sectionsList = sections.map(
    (
      { content, imageLink, traditional, isSection, reverse, contentBelow },
      idx
    ) => {
      return (
        <Fragment key={idx}>
          ...
          <div key={idx} className="container">
            ...
            >
              {window.innerWidth > 767 ? (
                ...
                <Fragment>
                  ...
                  >
                    {imageLink && <img src={imageLink} alt="section"></img>}
                  </div>
                  {!reverse && (
                    <div className="section-content">{content(place)}</div>
                  )}
            ...
            {contentBelow && <div className="contentBelow">{contentBelow}</div>}
          </div>
        </Fragment>
      );
    }
